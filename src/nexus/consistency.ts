 ...
 * **本模块只算不判**：阈值（≥50 对）、结论纪律与「留出集提升才采纳」都在调用方。
 *
...
/** 路由契约用的四字段读数（pairs<2 → null：一对样本给不出相关性，不给假读数）。 */
...
/**
 * 路由契约的四字段读数：`{pairs, exact, near, kappa}`；**配对 < 2 → null**
 * （一对样本的一致性恒为「完全一致」，报出来是假读数）。
 */
export function consistencyOf(pairs: readonly AlignPair[]): ConsistencySummary | null {
  if (pairs.length < 2) return null
  const m = metrics(pairs)
  return { pairs: m.n, exact: m.exact ?? 0, near: m.near ?? 0, kappa: m.kappa }
}

/**
 * @dsh-external/dsh-nautilus — fs.watch edit listener (main event source) + debounce merge.
...
export function startVaultWatch(store: NautilusStore, opts: WatchOptions): () => void {
  ...
  const settle = async (rel: string, windowKey: string): Promise<void> => {
    try {
      ...
      console.error('[nautilus] watch settle failed', rel, String(e))
    }
  }
  ...
  watcher.on('error', (err) => {
    console.warn('[nautilus] vault watch error (fallback to scan-only):', String(err))
  })

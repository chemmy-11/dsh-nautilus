...
This repository does not commit `lib/`, so a **git-form install builds in place at install time**: the `prepare` hook (`node scripts/prepare.mjs --if-dependency`) runs **only when the package is installed as a dependency** (cwd inside the consumer's `node_modules`), while a local `npm install` / `npm ci` never builds implicitly (use `npm run build`); the build probes `$DSH_CHECKOUT` / `~/dsh-harness` and falls back to npm-devDeps mode (devDependencies are installed by the package manager for git installs); pnpm ≥10 requires allowing `allowBuilds` in the profile's `pnpm-workspace.yaml` on first install — that authorizes running the package's build code at install time, so **pin a commit SHA**.

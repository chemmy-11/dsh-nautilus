...
- **工作台五视图**（全局面板：总览 / 曲线 / 假设 / 预言 / 报告）+ 逐轮抽屉；曲线支持**时间档位（1 周/1 月）、指标切换（未命中率 / 累计输入 / TPS / 时长）、轮次轴、等比放缩、平移、全屏、悬停简略读数与点击下钻完整问答**；
...
- 插件只有一个指向：**L 场指向**（会话归属的工作区根），在**工作台总览的「L 场读数（独立指向）」面板**内切换（历史归属不删）；vault 指向已随 vault 观测腿下线；
- **视图两态**同在该面板：`全局`（全部工作区）/ `指向`（当前指向工作区）——切换即改取数口径，曲线/假设/报告同步；

...
DeepSeek Harness（`dsh`）的观测插件（`@dsh-external/dsh-nautilus`）：为 Obsidian vault 与 AI 会话提供**双面板量化观测**——vault 侧元数据快照与编辑统计，会话侧逐轮遥测与形态分析。对 vault 只读，观测数据全部私有化存储（`~/.dsh/nautilus/`），重启/重载不丢不重。
...
| 端点 | 说明 |
|---|---|
| `GET /api/nautilus/state` | vault 总量 / 今日 / 本周 / 最近编辑流 |
| `GET/POST /api/nautilus/vault` | vault 指向状态 / 切换 |

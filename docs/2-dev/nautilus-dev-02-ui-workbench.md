...
> **2026-09-28 AL.4d 裁决（守谷人；覆盖先前 B 方案）**：打分件**撤下边界行**——人工侧不再采集 boundary；
> 但 POST body **仍恒带 boundary:'none'**（服务端「新形判据」的键，与 UI 一起删会让 N/A 落回 schema_version=1 的代际错行）。
> 后果如实标注：**人工侧 boundary 恒为 none，边界轴当前只有自评侧有信息**（边界面板注记已加此句）。
> 同批 UI：输入框由单行 input 改 **textarea（min-height 56px ≈ 改前 27px 的 2 倍）**。

> **2026-09-28 AL.4e 裁决（守谷人；覆盖上两轮的「右上角 + 6px 净空」）**：提交键与分数**在同一高度**——
> 回到「与 1–5 / N/A 同一行、垂直居中对齐」，仍在浮层首行的**右端**（视觉上仍是右上区域），
> 但**不再是一个浮在分值排上方的独立元素**。相应撤销为它预留的 36px 顶部净空（padding 回到常规 8px 9px，
> 浮层高度随之回落）；提交键去掉绝对定位、与档位键同规格，同高由分值排的 align-items:center 保证。
> **输入框高度翻倍（min-height 56px）不回退**；浮层在按钮上方、无边界行、body 仍带 boundary 判据键，均不变。

> **2026-09-28 AL.4f 裁决（守谷人）**：提交键**右端与输入框右端对齐**（两者内容盒右边线重合）。
> 做法：提交键加 margin-left:auto（在分值排内右推）+ 分值排 width:100%（对齐成立的前提——输入框本就是 width:100%，
> 且浮层左右内边距对称 8px 9px，故排右端 = 输入框右端）。
> 实测（无头 Edge，复现件）：submitRight 44 → **68**、inputRight **68**，右端差 24px → **0**；
> 同排同高（顶边同一值）· 7 个控件同一行不换行 · 浮层宽 292px 不变 · 输入框仍 56px。

> **2026-09-28 AL.5u（守谷人需求）**：工作台新增**「会话」视图**（位次：对齐之后）——
> **列表**：每行直接渲染服务端派生的 label（工作区名 · 会话名）+ 轮次数 + 时间范围 + 读数合计（in/out、cache、时长、平均 tps）
> + 已标注计数（人工 / 自评 / 旧代际**三路分列**，代际不混算）；点 label **展开逐轮**并可直接打分。
> **轮次命名统一**：「工作区名 · 会话名 · T<轮次>」（两段名取契约 /m2/sessions；索引缺失时按契约同款兜底：未知工作区 + 会话短形）——
> 对齐台账与会话逐轮行一律用此形式，**不再出现裸 session-xxxx:12 或 tNN**。
> **负载纪律**：未展开**不请求**详情接口；列表 50 条一页分页（契约未给总数，故「下一页」以「本页满 50 条」为判据，不编造总数）；
> 第 0 页复用根组件取数，其余页才自取。
> **原文缺失不打分**：hasText === false 的轮次**不渲染打分入口**，改显可见原因（服务端 no-turn-text 会拒）——不假装能打；
> 其余轮次复用会话内对齐打分件（写路径仍是 POST /m2/turn-annotations，带 align + boundary 判据键）。
> **打分后**只刷新该会话详情与该行计数（不整页重拉）。
> **诚实边界**：该端点**尚未随基线落地**（本提交时 git grep m2/sessions 零命中），故端上会显示「会话接口不可用」，
> 直至主线落地该契约 + 宿主重启；本轮证据 = SSR 断言 + 源码守卫 + 负控（不依赖活宿主）。

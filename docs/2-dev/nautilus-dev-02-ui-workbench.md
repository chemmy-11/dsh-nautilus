...
### 5.4 T 系列契合标注呈现（2026-09-27 已落地；口径 = 决策 D-T5，契约 = dev-05 §3）

与 §5.1–5.3 的「标签式标注」**并行的第二套轮次人工判读**（5 档契合量表，锁版 `schema_version=1`）：

| 呈现位 | 形态 | 纪律 |
|---|---|---|
| **流内契合条**（主入口） | 会话轮末动作行上方一条：5 档 + N/A chips、fit=4 引文框就地展开、`+理由`、已标回显（`已标 N` + sample 口径 `样` 徽标） | 挂宿主 `conversation.chat.turnTail` 链槽（select/inject 契约经 0.1.5-rc.2 运行时核实）；写路径同源 POST，**与读数零耦合** |

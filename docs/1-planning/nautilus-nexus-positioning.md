...
**nexus（层名）= Nautilus 的应用层腿：把宿主会话事件变成「应用层事实」，并保留自己独立的会话读数看板；对 Nautilus 只承担「被聚合」，不承担「聚合」。**
（2026-09-27 收窄：**vault 观测腿下线**——本层只做「会话事件 → 读数」，vault 侧操作归会话侧 /obsidian 技能。）
（实现这层的插件现名 **nautilus**：`@dsh-external/dsh-nautilus`——层名待裁决，见 D-N0。）

| 维度 | 做 | 不做 |
|---|---|---|
| 采集 | `session/event` 直采（turn 级 token/缓存/耗时/tps）、自评三行、问答全文 | 不采 OS/GPU（pulse）、不采推理引擎（infer）、不读 vault（2026-09-27 下线）、不互相打点 |
| 存储 | 自己的表、自己的口径、自己的迁移 | 不做指标注册中心、不做时间窗对齐、不做 era 判定、不做关联快照（全归 core） |
| 界面 | 工作台全局面板 + 逐会话「L 场读数」tab | 不承担 Nautilus 三层联动视图与归因报告页（L2） |
| 对外 | 只读查询面 + 已定稿的归属/指向语义 | 不碰 vault、不写他人数据目录、不 import 宿主实现或其它插件值 |
